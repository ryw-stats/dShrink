##wald confidence interval does not work because sig0b is nearly singular
...
simone <- function(sim) {
  ...
  G <- t(X) %*% (h0b * X) / n + diag(0.1, p) / n
  
  
  IF_beta <- - (- 4 * c(plogis(p_lin) * (1 - plogis(p_lin)) * (y_tilde - 2 * plogis(p_lin) + 1)) * X) %*% ginv(G)
  ...
  PC_H0$values <- Re(PC_H0$values)
  PC_H0$vectors <- Re(PC_H0$vectors)
  ...
  ITE_true <- c(2 * plogis(X_aug %*% beta) - 1)
  
  c(mean(ITE_true <= CI_tar[, 2] & ITE_true >= CI_tar[, 1]),
    mean(ITE_true <= CI_Prop[, 2] & ITE_true >= CI_tar[, 1]),
    mean(CI_tar[, 2] - CI_tar[, 1]),
    mean((CI_Prop[, 2] - CI_Prop[, 1])),
    EST_tar,
    EST_Prop
    )
}
...
ITE_beta0 <- Res[5:(4 + N), ]
ITE_ds <- Res[(5 + N):(4 + 2 * N), ]
ITE_true <- c(2 * plogis(X_aug %*% beta) - 1)

save(Res, file = "ITE_CI_Sim.Rdata")

rm(var)

Res_BV <- cbind(rowMeans(ITE_beta0 - ITE_true)^{2}, rowMeans(ITE_ds - ITE_true)^{2}, apply(ITE_beta0, 1, var), apply(ITE_ds, 1, var))

library(latex2exp)
library(ggplot2)

Res_BV <- data.frame(cbind(rep(c("TargetB2", "dShrinkB2", "TargetVar", "dShrinkVar"), c(N, N, N, N)), c(Res_BV)))
colnames(Res_BV) <- c("Method", "Value")
Res_BV$Method <- factor(Res_BV$Method, levels = c("TargetB2", "dShrinkB2", "TargetVar", "dShrinkVar"))
Res_BV$Value <- as.numeric(Res_BV$Value)


ggplot(Res_BV, aes(x=Method, y=Value, color=Method)) + # fill=name allow to automatically dedicate a color for each group
  geom_boxplot(outlier.shape = NA, fatten = 2, lwd=1.2) + 
  scale_x_discrete(labels = c(TeX('Target Pop Bias$^2$'), TeX('dShrink Bias$^2$'), TeX('Target Pop Var'), TeX('dShrink Var'))) +
  ylab("") +
  xlab("") +
  ylim(0, 0.04) +
  theme_light() +
...
ggsave(paste(paste("Boxplot", "ITE", "Bias", "Var", sep = "_"), ".pdf", sep = ""),
